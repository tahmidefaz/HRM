...
from puzzle_dataset_mlx import PuzzleDataset, PuzzleDatasetConfig, PuzzleDatasetMetadata
from utils.functions import load_model_class_mlx, get_model_source_path_mlx
from models.sparse_embedding_mlx import SparseEmbeddingSignSGD
...
def create_model(config: PretrainConfig, train_metadata: PuzzleDatasetMetadata):
    ...
    model_cls = load_model_class_mlx(config.arch.name)
    loss_head_cls = load_model_class_mlx(config.arch.loss.name)
...
def save_code_and_config(config: PretrainConfig):
    ...
    # Copy code
    code_list = [
        get_model_source_path_mlx(config.arch.name),
        get_model_source_path_mlx(config.arch.loss.name)
    ]
